...
import { handleLockRotationToFace } from "@/replay-tree-manager/entity-motion-update-handlers/handle-lock-rotation-to-face";
...
export function threatTargetChangedIndicatorSequence() {
  ...
  for (const combatant of party.combatantManager.getAllCombatants()) {
    ...
    const monsterCharacterModel = getGameWorldView().modelManager.findOne(combatant.getEntityId());
    ...
    getGameWorldView().actionEntityManager.register(indicatorArrow);

    const newTargetId = threatManager.getHighestThreatCombatantId();
    if (newTargetId === null) continue;

    const newTargetCharacterModel = getGameWorldView().modelManager.findOne(newTargetId);
...
function spawnTargetChangedIndicatorArrow(position: Vector3) {
  ...
  model.meshes = [mesh];
  const entityId = getGameWorldView().idGenerator.generate();
