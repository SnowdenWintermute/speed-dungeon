import { CombatActionName, DungeonRoomType, GameServer, LobbyServer } from "@speed-dungeon/common";
...
describe.each(TEST_CONNECTION_ENDPOINT_FACTORIES)(
  ...
  ({ clientEndpointFactory }) => {
    ...
    let gameClientHarness: ClientTestHarness;
    let clientApplication: ClientApplication;
    const timeMachine = new TimeMachine();

    beforeEach(async () => {
      const setup = await enterTestGameSingleCharacter(clientEndpointFactory, timeMachine);
