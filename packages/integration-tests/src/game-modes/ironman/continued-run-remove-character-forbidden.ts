...
export async function testContinuedRunRemoveCharacterForbidden(
  ...
  expect(game.id).toBe(alphaIronmanRunRef.gameId);
  ...
  invariant(alphaCharacterId !== undefined, "expected alpha to own a character in this game");

  await alpha.lobbyClientHarness.deleteCharacterInGame(alphaCharacterId);
