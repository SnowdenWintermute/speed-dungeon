  ...
  getCosmeticEffectsToStart: (context) => {
    return [
      CosmeticEffectInstructionFactory.createParticlesOnOffhand(
        CosmeticEffectNames.RedParticleAccumulation,
        context
      ),
    ];
  },
...
finalStepOverrides[ActionResolutionStepType.RecoveryMotion] = {
  getCosmeticEffectsToStart: (context) => {
    ...
    const toReturn: CosmeticEffectOnTargetTransformNode[] = targetIdsResult.map((targetId) => {
      return {
        name: CosmeticEffectNames.ProvokeCast,
        lifetime: 700,
        parent: {
...
invariant(finalPositioningStep !== undefined, "expected finalPositioningStep");
delete finalPositioningStep.getAnimation;
finalPositioningStep.shouldIdleOnComplete = true;

export const PROVOKE_STEPS_CONFIG = stepsConfig;
