import cloneDeep from "lodash.clonedeep";
import { CombatActionTargetingPropertiesConfig } from "../../../combat-action-targeting-properties.js";
import {
  PET_OF_USER_TARGETING_PROPERTIES,
  SINGLE_ANY_TARGETING_PROPERTIES,
  SINGLE_FRIENDLY_TARGETING_PROPERTIES,
  SINGLE_HOSTILE_TARGETING_PROPERTIES,
} from "./single.js";
import {
  AREA_ANY_TARGETING_PROPERTIES,
  AREA_FRIENDLY_TARGETING_PROPERTIES,
  AREA_HOSTILE_TARGETING_PROPERTIES,
} from "./area.js";
import { COPY_PARENT_HOSTILE_TARGETING_PROPERTIES } from "./copy-parent.js";
import {
  SELF_ANY_TIME_TARGETING_PROPERTIES,
  SELF_IN_COMBAT_TARGETING_PROPERTIES,
  SELF_OUT_OF_COMBAT_TARGETING_PROPERTIES,
} from "./self.js";
import { EXPLOSION_TARGETING_PROPERTIES_CONFIG } from "./explosion.js";

export const TARGETING_PROPERTIES_TEMPLATE_GETTERS = {
  SINGLE_HOSTILE: () => cloneDeep(SINGLE_HOSTILE_TARGETING_PROPERTIES),
  SINGLE_FRIENDLY: () => cloneDeep(SINGLE_FRIENDLY_TARGETING_PROPERTIES),
  SINGLE_ANY: () => cloneDeep(SINGLE_ANY_TARGETING_PROPERTIES),
  AREA_HOSTILE: () => cloneDeep(AREA_HOSTILE_TARGETING_PROPERTIES),
  AREA_FRIENDLY: () => cloneDeep(AREA_FRIENDLY_TARGETING_PROPERTIES),
  AREA_ANY: () => cloneDeep(AREA_ANY_TARGETING_PROPERTIES),
  COPY_PARENT_HOSTILE: () => cloneDeep(COPY_PARENT_HOSTILE_TARGETING_PROPERTIES),
  SELF_IN_COMBAT: () => cloneDeep(SELF_IN_COMBAT_TARGETING_PROPERTIES),
  SELF_OUT_OF_COMBAT: () => cloneDeep(SELF_OUT_OF_COMBAT_TARGETING_PROPERTIES),
  SELF_ANY_TIME: () => cloneDeep(SELF_ANY_TIME_TARGETING_PROPERTIES),
  EXPLOSION: () => cloneDeep(EXPLOSION_TARGETING_PROPERTIES_CONFIG),
  PET_OF_USER: () => cloneDeep(PET_OF_USER_TARGETING_PROPERTIES),
};

export function createTargetingPropertiesConfig(
  templateGetter: () => CombatActionTargetingPropertiesConfig,
  overrides: Partial<CombatActionTargetingPropertiesConfig>
): CombatActionTargetingPropertiesConfig {
  const base = templateGetter();

  return {
    ...base,
    ...overrides,
  };
}
