...
//   1. LadderUpdatePolicy has no records-service dependency. Its constructor injects ExperiencePointsLadderService (the XP/level leaderboard — a
...
//   the per-scheme XP leaderboards (ExperiencePointsLadderService), not the records — our records carry control_scheme as a column, so one table
