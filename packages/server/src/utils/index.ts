...
export function generateRandomGameName() {
  const firstName =
    RANDOM_GAME_NAMES_FIRST[
      Math.floor(Math.random() * RANDOM_GAME_NAMES_FIRST.length)
    ];
  const lastName =
    RANDOM_GAME_NAMES_LAST[
      Math.floor(Math.random() * RANDOM_GAME_NAMES_LAST.length)
    ];
  return `${firstName} ${lastName}`;
}

export function generateRandomPartyName() {
  return RANDOM_PARTY_NAMES[
    Math.floor(Math.random() * RANDOM_PARTY_NAMES.length)
  ];
}
