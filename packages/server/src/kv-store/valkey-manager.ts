...
export class ValkeyManager {
  ...
  async connect() {
    ...
    const timeout = new Promise<never>((_, reject) => {
      connectionTimeout = setTimeout(
        () => reject(new Error("connection to valkey timed out")),
  ...
  async hSet(key: string, field: string, value: string) {
    return this.client.hSet(this.keyPrefix + key, field, value);
  }
  async hGet(key: string, field: string) {
    return this.client.hGet(this.keyPrefix + key, field);
  }
  ...
  async zRange(key: string, min: number, max: number, options: Object) {
    return this.client.zRange(this.keyPrefix + key, min, max, options);
  }
